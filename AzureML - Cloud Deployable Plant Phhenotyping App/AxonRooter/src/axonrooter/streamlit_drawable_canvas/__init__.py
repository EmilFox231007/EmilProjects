...
# Flag to differentiate between development and production modes.
# When packaging the app for release, this should be set to True.
_RELEASE = True

if not _RELEASE:
    # Declare the Streamlit component for local development, served via localhost
    _component_func = components.declare_component(
        "st_canvas",
        url="http://localhost:3001",
    )
else:
    # Declare the Streamlit component for production, using the built frontend assets
...
class CanvasResult:
    """
    Container for the output returned by the canvas component.

    Attributes
    ----------
    image_data : np.ndarray or None
        The RGBA image data from the canvas as a NumPy array
        of shape (height, width, 4).
    json_data : dict or None
        JSON dictionary describing the canvas objects and metadata.
    """
...
def _data_url_to_image(data_url: str) -> Image:
    """
    Convert a base64-encoded data URL string to a PIL Image.

    Parameters
    ----------
    data_url : str
        Data URL containing base64 encoded image data
        in the format "data:image/png;base64,...".
...
def _resize_img(img: Image, new_height: int = 700, new_width: int = 700) -> Image:
    ...
    new_height : int, optional
        Desired height in pixels. Default is 700.
    new_width : int, optional
        Desired width in pixels. Default is 700.
